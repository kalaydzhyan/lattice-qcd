...
for($i=0; $i<$nconfigs; $i++)
{
 ...
 $config    = $configs[$i];
 $runfname  = sprintf("%s/run_ovd_%03id_H%i.sh", $basedir ,$i, $H);
 ...
 $wdmarkname   = sprintf("wd_%03id_s%i_t%i_H%i", $i, $LS, $LT, $H);
 ...
 printf RUNFILE "scp buividovich\@rrcsrv.itep.ru:$scpdir/wd_%03id_s%i_t%i_H%i.dat $datadir1\n", $i, $LS, $LT, $H;
...
# system("echo $nmarks");
 ...
 system("qsub -cwd -l arch=x86 -l h_vmem=3G  $runfname");
 sleep(15);
};
